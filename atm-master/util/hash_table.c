...
void hash_table_add(HashTable *ht, char *key, void *val)
{
    ...
    {
        char * safeKey = (char *) calloc(strlen(key), sizeof(char));
        strncpy(safeKey, key, strlen(key));
        char * safeValue = (char *) calloc(strlen(val), sizeof(char));
        strncpy(safeValue, val, strlen(val));
        ht->size -= list_size(ht->bins[idx]);
        list_add(ht->bins[idx], safeKey, safeValue);
        ht->size += list_size(ht->bins[idx]);
    }
}
